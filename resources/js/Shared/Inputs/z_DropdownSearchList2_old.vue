<template>
    <label v-if="label" class="font-medium text-sm text-gray-700" :class="error ? 'text-red-700' : ''">{{ label }}:</label>
    <div class="dropdown">
...
export default {
    ...
    methods: {
        ...
        selectItem(theItem) {
            ...
            this.$emit('update:modelValue', theItem.id)
            this.$emit('itemDetail', theItem)
        },
        itemVisible(item) {
            let perperprr = "ererer"
        ...
        getList() {
            axios.get(this.apiUrl).then(response => {
                ...
                if (this.modelValue) {
                    this.selectedItem = this.itemList.find(x => x.id === this.modelValue)
                }
            })
        },
        addItem() {
